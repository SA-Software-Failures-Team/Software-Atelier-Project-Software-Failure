...
* All `git commit` and `git push` commands **MUST** be performed from the Terminal. **DON’T** use the GitHub website interface to upload or edit files.
...
## 2. First-Time Setup: How to Clone the Repository

Before you can work on the project, you need to copy it (clone it) to your computer. You only need to do this **ONCE**.

1.  **Install Git: (ONLY IF YOU DIDN’T FOLLOW GIT HANDSON)** If you don't have it, go back to GIT Lecture and Hands On pdf’s on iCorsi and install Git from accordingly.
2.  **Configure Git:(ONLY IF YOU DIDN’T FOLLOW GIT HANDSON)** Open your terminal and set your name and email. This is required for the project grading.
    ```bash
