document.addEventListener('DOMContentLoaded', function () {
    ...
    <header class="p-3">
        <div class="container-fluid">
            <div class="d-flex flex-wrap align-items-center justify-content-center justify-content-lg-start">
                ...
                <div class="bta exit" style="font-size: 15px;">Go Back</div>
            </div>
        </div>
    </header>
    `;
    ...
    if (topicFolder && subtopicFolder) {
        document.querySelectorAll('.sidebar a').forEach(link => {
            ...
            if (linkHref.includes(`/${topicFolder}/`) && linkHref.includes(`/${subtopicFolder}/`)) {
    ...
    for (let i = 0; i < divs.length; i++) {
        divs[i].onclick = function () {
            ...
            let target = (sessionStorage.getItem("category") === "bug")
                ? base + "/topicgeneral_b.html"
                : base + "/topicgeneral_v.html";
    ...
    function openSidebar() {
        ...
        const activeLink = sidebar.querySelector('.current-page-link');
        if (activeLink) {
    ...
    document.querySelectorAll('[data-toggle]').forEach(item => {
        item.addEventListener('click', function (e) {
            ...
            if (this.tagName === 'A' && !isArrowArea) return;
